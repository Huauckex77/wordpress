jQuery( document ).ready(function($) {
	...
	jQuery(document).ready(function ($) {
		...
		$('body').on('click', '#sub-accordion-panel-bizes_frontpage_settings .control-subsection .accordion-section-title', function(event) {
	...
	function scrollToSection( section_id ){
		...
		var preview_section_id = "hero-slider";
		...
		if(numRepeaterItems > 0) {
			...
			if(numRepeaterItems > 1) {
				...
				for (i = 1; i < numRepeaterItems; ++i) {
					bizesAppendRow($(this), defaultValuesArray[i]);
				}
			}
		}
	});
	...
	$('.sortable_repeater.sortable').on('click', '.customize-control-sortable-repeater-delete', function(event) {
		...
		if(numItems > 1) {
			$(this).parent().slideUp('fast', function() {
				...
				$(this).remove();
				bizesGetAllInputs(parentContainer);
			})
		}
		else {
			$(this).parent().find('.repeater-input').val('');
			bizesGetAllInputs($(this).parent().parent().parent());
		}
	});
	...
	function bizesAppendRow($element, defaultValue = '') {
	...
	function bizesGetAllInputs($element) {
	...
	$('.multi-image-checkbox').on('change', function () {
	  bizesGetAllImageCheckboxes($(this).parent().parent());
	});

	// Get the values from the checkboxes and add to our hidden field
	function bizesGetAllImageCheckboxes($element) {
	...
	$( ".pill_checkbox_control .sortable" ).sortable({
		...
		update: function( event, ui ) {
			bizesGetAllPillCheckboxes($(this).parent());
		}
	});

	$('.pill_checkbox_control .sortable-pill-checkbox').on('change', function () {
		bizesGetAllPillCheckboxes($(this).parent().parent().parent());
	});

	// Get the values from the checkboxes and add to our hidden field
	function bizesGetAllPillCheckboxes($element) {
	...
	$('.google-fonts-list').on('change', function() {
		...
		var indexes = $.map(bodyfontcontrol.bizesfontslist, function(obj, index) {
		...
		$.each(bodyfontcontrol.bizesfontslist[index].variants, function(val, text) {
		...
		$(this).parent().parent().find('.google-fonts-category').val(bodyfontcontrol.bizesfontslist[index].category);

		bizesGetAllSelects($(this).parent().parent());
	});
	...
	function bizesGetAllSelects($element) {
		var selectedFont = {
	...
	$('.customize-control-tinymce-editor').each(function(){
		...
		var tinyMCEToolbar1String = _wpCustomizeSettings.controls[$(this).attr('id')].bizestinymcetoolbar1;
		var tinyMCEToolbar2String = _wpCustomizeSettings.controls[$(this).attr('id')].bizestinymcetoolbar2;
		var tinyMCEMediaButtons = _wpCustomizeSettings.controls[$(this).attr('id')].bizesmediabuttons;
 ...
 */
( function( $, api ) {
	api.sectionConstructor['bizes-upsell'] = api.Section.extend( {
