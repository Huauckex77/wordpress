<?php
/**
 * Bizes Customizer Custom Controls
 *
 */
...
if ( class_exists( 'WP_Customize_Control' ) ) {
	 ...
	 */
	class Bizes_Custom_Control extends WP_Customize_Control {
		protected function get_bizes_resource_url() {
	...
	if( ! class_exists( 'Bizes_Dropdown_Taxonomies_Control' ) ):
		class Bizes_Dropdown_Taxonomies_Control extends WP_Customize_Control{
			private $cats = false;
	 ...
	 * @link https://github.com/maddisondesigns
	 */
	class Bizes_Custom_Section extends WP_Customize_Section {
		protected function get_bizes_resource_url() {
			if( strpos( wp_normalize_path( __DIR__ ), wp_normalize_path( WP_PLUGIN_DIR ) ) === 0 ) {
				// We're in a plugin directory and need to determine the url accordingly.
				return plugin_dir_url( __DIR__ );
	 ...
	 * @link https://github.com/maddisondesigns
	 */
	 class Bizes_Image_Checkbox_Custom_Control extends Bizes_Custom_Control {
		/**
	 ...
	 * @link https://github.com/maddisondesigns
	 */
	class Bizes_Single_Accordion_Custom_Control extends Bizes_Custom_Control {
		/**
		 * The type of control being rendered
		 */
	 ...
	 * @link https://github.com/maddisondesigns
	 */
	class Bizes_Simple_Notice_Custom_Control extends Bizes_Custom_Control {
		/**
		 * The type of control being rendered
		 */
	 ...
	 * @link https://github.com/maddisondesigns
	 */
	class Bizes_Sortable_Repeater_Custom_Control extends Bizes_Custom_Control {
		/**
		 * The type of control being rendered
		 */
	 ...
	 * @link https://github.com/maddisondesigns
	 */
	class Bizes_TinyMCE_Custom_control extends Bizes_Custom_Control {
		/**
		 * The type of control being rendered
		 */
		...
		public function to_json() {
			parent::to_json();
			$this->json['bizestinymcetoolbar1'] = isset( $this->input_attrs['toolbar1'] ) ? esc_attr( $this->input_attrs['toolbar1'] ) : 'bold italic bullist numlist alignleft aligncenter alignright link';
			$this->json['bizestinymcetoolbar2'] = isset( $this->input_attrs['toolbar2'] ) ? esc_attr( $this->input_attrs['toolbar2'] ) : '';
			$this->json['bizesmediabuttons'] = isset( $this->input_attrs['mediaButtons'] ) && ( $this->input_attrs['mediaButtons'] === true ) ? true : false;
		}
		/**
		 * Render the control in the customizer
		 */
	...
	class Bizes_Google_Font_Select_Custom_Control extends Bizes_Custom_Control {
		...
		public function render_content() {
			...
			if( !empty($this->fontList) ) {
				...
				<div class="google_fonts_select_control">
					...
					<div class="customize-control-description"><?php esc_html_e( 'Font Weight', 'bizes' ) ?></div>
		 ...
		 */
		public function bizes_getFontIndex( $haystack, $needle ) {
		 ...
		 */
		public function bizes_getGoogleFonts( $count = 30 ) {
			// Google Fonts json generated from https://www.googleapis.com/webfonts/v1/webfonts?sort=popularity&key=YOUR-API-KEY
			$fontFile = $this->get_bizes_resource_url() . 'controls/google-fonts-alphabetical.json';
			if ( $this->fontOrderBy === 'popular' ) {
				$fontFile = $this->get_bizes_resource_url() . 'controls/google-fonts-popularity.json';
			}
	 ...
	 */
	class Bizes_Customize_Alpha_Color_Control extends Bizes_Custom_Control {
		/**
		 * The type of control being rendered
		 */
		 ...
		 */
		public function enqueue() {
			wp_enqueue_script( 'bizes-custom-controls-js', $this->get_bizes_resource_url() . 'assets/js/customizer.js', array( 'jquery', 'wp-color-picker' ), '1.0', true );
			wp_enqueue_style( 'bizes-custom-controls-css', $this->get_bizes_resource_url() . 'assets/css/customizer.css', array( 'wp-color-picker' ), '1.0', 'all' );
		}
		/**
		 * Render the control in the customizer
		 */
	 ...
	 */
	class Bizes_Alpha_Color_Control extends Bizes_Custom_Control {
		/**
		 * The type of control being rendered
		 */
		...
		public function enqueue() {
			wp_enqueue_style( 'bizes-custom-controls-css', $this->get_bizes_resource_url() . 'assets/css/customizer.css', array(), '1.0', 'all' );
			wp_enqueue_script( 'wp-color-picker-alpha', $this->get_bizes_resource_url() . 'assets/js/wp-color-picker-alpha-min.js', array( 'wp-color-picker' ), '1.0', true );
	...
	class Bizes_Upsell_Section extends Bizes_Custom_Section {
		...
		protected function render() {
			...
			?>
			<li id="accordion-section-<?php echo esc_attr( $this->id ); ?>" class="bizes_upsell_section accordion-section control-section control-section-<?php echo esc_attr( $this->id ); ?> cannot-expand">
	 ...
	 *
	 */
	class Bizes_Misc_Control extends WP_Customize_Control {
	 ...
	 */
	if ( ! function_exists( 'bizes_url_sanitization' ) ) {
		function bizes_url_sanitization( $input ) {
	 ...
	 */
	if ( ! function_exists( 'bizes_switch_sanitization' ) ) {
		function bizes_switch_sanitization( $input ) {
			if (true === $input ) {
				return 1;
			} else {
				return 0;
	 ...
	 * @return integer	Sanitized value
	 */
	if ( ! function_exists( 'bizes_radio_sanitization' ) ) {
		function bizes_radio_sanitization( $input, $setting ) {
	 ...
	 */
	if ( ! function_exists( 'bizes_sanitize_integer' ) ) {
		function bizes_sanitize_integer( $input ) {
			return (int) $input;
		}
	}
	 ...
	 * @return string	Sanitized input
	 */
	if ( ! function_exists( 'bizes_text_sanitization' ) ) {
		function bizes_text_sanitization( $input ) {
			if ( strpos( $input, ',' ) !== false) {
				$input = explode( ',', $input );
			}
			if( is_array( $input ) ) {
	 ...
	 */
	if ( ! function_exists( 'bizes_array_sanitization' ) ) {
		function bizes_array_sanitization( $input ) {
			if( is_array( $input ) ) {
				foreach ( $input as $key => $value ) {
					$input[$key] = sanitize_text_field( $value );
				}
	 ...
	 * @return string	Sanitized input
	 */
	if ( ! function_exists( 'bizes_hex_rgba_sanitization' ) ) {
		function bizes_hex_rgba_sanitization( $input, $setting ) {
			...
			if ( false === strpos( $input, 'rgb' ) ) {
				...
				if ( false === strpos( $input, 'rgba' ) ) {
					...
					$input = 'rgb(' . bizes_in_range( $red, 0, 255 ) . ',' . bizes_in_range( $green, 0, 255 ) . ',' . bizes_in_range( $blue, 0, 255 ) . ')';
				}
				else {
					// Sanitize as RGBa color
					$input = str_replace( ' ', '', $input );
					sscanf( $input, 'rgba(%d,%d,%d,%f)', $red, $green, $blue, $alpha );
					$input = 'rgba(' . bizes_in_range( $red, 0, 255 ) . ',' . bizes_in_range( $green, 0, 255 ) . ',' . bizes_in_range( $blue, 0, 255 ) . ',' . bizes_in_range( $alpha, 0, 1 ) . ')';
				}
			}
			return $input;
		}
	 ...
	 */
	if ( ! function_exists( 'bizes_in_range' ) ) {
		function bizes_in_range( $input, $min, $max ){
			if ( $input < $min ) {
				$input = $min;
			}
			if ( $input > $max ) {
	 ...
	 * @return string	Sanitized input
	 */
	if ( ! function_exists( 'bizes_google_font_sanitization' ) ) {
		function bizes_google_font_sanitization( $input ) {
	 ...
	 * @return string	Sanitized input
	 */
	if ( ! function_exists( 'bizes_date_time_sanitization' ) ) {
		function bizes_date_time_sanitization( $input, $setting ) {
	 ...
	 * @return string	Sanitized input
	 */
	if ( ! function_exists( 'bizes_range_sanitization' ) ) {
		function bizes_range_sanitization( $input, $setting ) {
			...
			return bizes_in_range( $number, $min, $max );
		}
	}

		// Number Float-val
	if ( ! function_exists( 'bizes_floatval_sanitization' ) ) {
		function bizes_floatval_sanitization($input)
		{
...
if ( ! function_exists( 'bizes_validate_duration' ) ) :
	/**
	 ...
	 */
	function bizes_validate_duration( $validity, $value ){
		$value = intval( $value );
		if ( empty( $value ) || ! is_numeric( $value ) ) {
			$validity->add( 'required', esc_html__( 'You must supply a valid number.', 'bizes' ) );
		} elseif ( $value < 1000 ) {
			$validity->add( 'min_slider', esc_html__( 'Minimum time of duration is 1000', 'bizes' ) );
		} elseif ( $value > 20000 ) {
			$validity->add( 'max_slider', esc_html__( 'Maximum time of duration is 20000', 'bizes' ) );
		}
		return $validity;
	}
endif;